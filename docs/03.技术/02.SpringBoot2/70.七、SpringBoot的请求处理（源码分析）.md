---
title: 七、SpringBoot的请求处理（源码分析）
date: 2023-05-15 14:47:55
permalink: /pages/34c966/
categories:
  - 技术
  - SpringBoot2
tags:
...
## 28、请求处理-【源码分析】-请求映射原理
![20210205005703527](https://cdn.statically.io/gh/747721653/picx-images-hosting@master/springboot/20210205005703527.2l0z2oj2aq20.png)
...
![20210205005802305](https://cdn.statically.io/gh/747721653/picx-images-hosting@master/springboot/20210205005802305.1lipq3ujdji8.webp)
其中，保存了所有`@RequestMapping` 和`handler`的映射规则。

![20210205005926474](https://cdn.statically.io/gh/747721653/picx-images-hosting@master/springboot/20210205005926474.3pp1a3jca000.webp)
...
使用`@CookieValue`注解时可以直接将值存入Cookie对象中
...
两种方式能够拿到请求域中的数据：一种是通过`@RequestAttribute`，另一种是使用`HttpServletRequest`对象获取
...
2. SpringBoot默认是禁用了矩阵变量的功能
    - 手动开启：原理。对于路径的处理。`UrlPathHelper`的`removeSemicolonContent`设置为false，让其支持矩阵变量的。

3. 矩阵变量**必须**有url路径变量才能被解析

   且路径变量存储的值为分号之前的部分，例如`cars/sell;low=34;brand=byd,audi,yd`在`/cars/{path}`中为path路径变量注入的值为sell

   若一个url中同时有两个路径变量，则需要在`@MatrixVariable`注解中加入`pathVar`说明具体是哪个路径变量中的
...
![20210205010047654](https://cdn.statically.io/gh/747721653/picx-images-hosting@master/springboot/20210205010047654.5oae3enzrjc0.webp)
...
`DispatchServlet`中的`doDispatch()`方法中调用`handleradapter`执行`handle()`方法
...
![20210205010247689](https://cdn.statically.io/gh/747721653/picx-images-hosting@master/springboot/20210205010247689.2968bre43nok.webp)
