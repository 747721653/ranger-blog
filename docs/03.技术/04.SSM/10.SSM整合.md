---
title: SSM整合
date: 2023-05-06 20:11:50
permalink: /pages/5b6ab2/
categories:
  - 技术
  - SSM
tags:
...
之所以使用监听器而不是过滤器是因为我们的IOC容器只需要在初始化的时候创建一次，不需要在运行的时候多次创建，而过滤器会对每次的请求都做判断，因此这里使用监听器即可；且监听器创建比DispatchServlet初始化要早
...
监听器ContextLoaderListener中有创建IOC容器的代码
...
<!-- 配置默认的servlet处理静态资源，当DispatchServlet处理不了某些请求时将交给默认Servlet处理-->
<mvc:default-servlet-handler />
<!-- 开启MVC的注解驱动，开启之后将首先使用DispatchServlet处理请求-->
...
核心配置文件中的配置都可以交给spring进行管理，即在spring.xml中SqlSessionFactoryBean的属性中进行设置
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <!--
		...
		配置了这个的话会同时根据设置的包名查找mapper接口和mapper映射文件
    -->
...
**url：**localhost:8080/employee/page/1
...
public PageInfo<Employee> getEmployeePage(Integer pageNum) {
    //开启分页功能
    PageHelper.startPage(pageNum, 4);
    //查询所有数据，这里实际上并没有查询所有数据，因为分页插件是一个拦截器，它会帮助我们加上limit，从而获取相应的部分数据
    List<Employee> list = employeeMapper.getAllEmployee();
    //获取分页相关数据，这里的5代表分页栏显示几条页码
